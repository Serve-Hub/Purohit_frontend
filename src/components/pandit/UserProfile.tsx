...
function UserProfile({user}) {
  ...
  return (
    ...
    <div className="relative z-9999 h-35 md:h-65">
      {/* {console.log("data is ")} */}
      <img
        src={user?.coverPhoto}
        alt="profile cover"
        className="rounded-tl-[10px] rounded-tr-[10px] object-cover object-center border-b border-orange-200"
...
<LetterAvatar name={user?.firstName } />
      ...
      <div className="mt-4">
        ...
        <p className="font-medium text-slate-400 text-sm"> </p>

        <div className="text-user mx-auto mb-5.5 mt-5 grid max-w-[600px] grid-cols-3 rounded-[5px] border border-orange-200 py-[9px] shadow-1 dark:border-dark-3 dark:bg-dark-2 dark:shadow-card">
  ...
  <div className="flex space-x-4 border-b border-gray-300 dark:border-gray-700">
    <button
      className={`py-2 px-4 text-sm font-medium ${activeTab === 'about' ? 'text-user dark:text-orange-400' : 'text-gray-500 dark:text-gray-400'}`}
      onClick={() => setActiveTab('about')}
    >
      About Me
    </button>
    <button
      className={`py-2 px-4 text-sm font-medium ${activeTab === 'reviews' ? 'text-user dark:text-orange-400' : 'text-gray-500 dark:text-gray-400'}`}
