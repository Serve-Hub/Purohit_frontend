...
// console.log("üserinfo is",userInfo)
    ...
    const fetchPanditData = async () => {
      ...
      try {
        console.log("pandit id is",user?._id);
        const panditId=user._id;
          const res=await $axios.put(`/api/v1/booking/panditDetails/${panditId}`)
        // const response = await loginfo();
        console.log("pandit info  is", res);
        setPanditDetails(res.data.data);
  ...
  {user?.avatar ? (
    ...
    <LetterAvatar name={user?.firstName } />
          ...
          <div className="mt-4">
            ...
            <div className="text-pandit mx-auto mb-5.5 mt-5 grid max-w-[600px] grid-cols-2 rounded-[5px] border border-orange-200 py-[9px] shadow-1 dark:border-dark-3 dark:bg-dark-2 dark:shadow-card">
