...
function PanditProfile({pandit}) {
    console.log("pandit is",pandit)
...
<LetterAvatar name={pandit?.firstName } />
)}
</div>
      </div>
      <div className="mt-4">
        ...
        <p className="font-medium text-slate-400 text-sm">{pandit?.email}</p>
        <p className="font-medium text-slate-400 text-sm"> {pandit.panditKYP[0].temporaryAddress.tolAddress } {pandit.panditKYP[0].temporaryAddress.municipality}</p>
  ...
  <div className="flex space-x-4 border-b border-gray-300 dark:border-gray-700">
    <button
      className={`py-2 px-4 text-sm font-medium ${activeTab === 'about' ? 'text-pandit dark:text-orange-400' : 'text-gray-500 dark:text-gray-400'}`}
